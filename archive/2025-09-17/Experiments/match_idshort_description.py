...
def run_match_idshort_description(aas_env: Dict[str, Any], prompts: List[Dict[str, Any]], llm_runner) -> List[Dict[str, str]]:
    properties = extract_properties(aas_env)
    rows: List[Dict[str, str]] = []

    logger.info(f"Running match_idshort_description on {len(properties)} properties")
    ...
    logger.info(f"Completed match_idshort_description for {len(rows)} properties")
    return rows
