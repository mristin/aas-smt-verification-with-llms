...
def main() -> int:
    ...
    for take in tqdm.tqdm(pending_takes):
        ...
        prompt = (
            f"For which of the following elements does its description "
            f"DOES NOT correspond with its display name?\n\n"
            ...
            f"Only compare the description with the display name and ignore the ID-short!\n"
            f"Note, that there are cases where there is no mismatch and return this as well.\n\n"
            f"{relevant_details}"
        )
