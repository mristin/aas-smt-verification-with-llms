...
def main() -> None:
    ...
    init_version = aas_smt_verification_with_llms.__version__

    pyproject_toml_version = importlib.metadata.version(
        "aas-smt-verification-with-llms"
    )

    if init_version != pyproject_toml_version:
        print(
            f"The version in {aas_smt_verification_with_llms.__file__} is: {init_version!r}, "
