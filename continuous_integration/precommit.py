...
def main() -> int:
    ...
    if Step.REFORMAT in selects and Step.REFORMAT not in skips:
        print("Re-formatting...")
        reformat_targets = [
            "aas_smt_verification_with_llms",
    ...
    if Step.DOCTEST in selects and Step.DOCTEST not in skips:
        ...
        for pth in (repo_root / "aas_smt_verification_with_llms").glob("**/*.py"):
    ...
    if (
        ...
        print(
            "Checking that aas_smt_verification_with_llms/__init__.py "
            "and pyproject.toml coincide..."
        )
        exit_code = call_and_report(
            verb=(
                "check that aas_smt_verification_with_llms/__init__.py "
                "and pyproject.toml coincide"
            ),
            cmd=[
                sys.executable,
                "continuous_integration/check_version_consistent.py",
            ],
            cwd=repo_root,
        )
        if exit_code != 0:
        ...
        print(
            "Skipped checking that aas_smt_verification_with_llms/__init__.py "
            "and pyproject.toml coincide..."
        )

    # NOTE (mristin):
    # We need to check for the Python version since ``argparse`` output changes
    # between the versions. Hence, we pin it at the moment to Python 3.10.
    if (3, 10) <= sys.version_info < (3, 11):
        ...
        print(
            ...
            "since we pin it on Python version 3.10."
        )
